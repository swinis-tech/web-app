...
interface Setting extends Partial<MethodParams> {
  asr: string;
  imsak: string;
  dhuhr: string;
  highLats: string
...
class PrayTimes {
  ...
  constructor(method?: keyof Methods) {
    this.calcMethod = method || this.calcMethod;
    const params = this.methods[this.calcMethod].params;
    for (const id in params) {
      // @ts-ignore
      this.setting[id as keyof MethodParams] = params[id as keyof MethodParams];
    }
  }
  // ----------------------- Public Functions ------------------------
  ...
  getTimes(date: Date | number[], coords: [number, number, number?], timezone?: 'auto' | number, dst?: 'auto' | number, format?: string) {
    this.lat = 1 * coords[0];
    this.lng = 1 * coords[1];
  ...
  getFormattedTime(time: number, format: string, suffixes: string[]) {
    if (isNaN(time)) {
  ...
  sunAngleTime(angle: number, time: number, direction?: string) {
    ...
    const t = 1 / 15 * DMath.arccos((-DMath.sin(angle) - DMath.sin(decl) * DMath.sin(this.lat as number)) /
      (DMath.cos(decl) * DMath.cos(this.lat as number)));
  ...
  tuneTimes(times: { imsak?: number; fajr?: number; sunrise?: number; dhuhr?: number; asr?: number; sunset?: number; maghrib?: number; isha?: number; midnight?: number | undefined; }) {
    for (const i in times) {
      // @ts-ignore
  ...
  modifyFormats(times: { imsak?: number; fajr?: number; sunrise?: number; dhuhr?: number; asr?: number; sunset?: number; maghrib?: number; isha?: number; midnight?: number | undefined; }) {
    for (const i in times) {
      // @ts-ignore
  ...
  dayPortion(times: { [x: string]: number; imsak?: any; fajr?: any; sunrise?: any; dhuhr?: any; asr?: any; sunset?: any; maghrib?: any; isha?: any; }) {
    for (const i in times) {
      times[i] /= 24;
    }
    return times;
